...
public class T01AVLTree {
    ...
    Node deleteNode(Node nd, int key) {
        ...
        if (nd.key > key)
            ...
            nd.right = deleteNode(nd.right, key);
        else {
        ...
        if (balance < -1 && nd.bal(nd.left) <= 0)
            return rightRotate(nd);
        if (balance < -1 && nd.bal(nd.left) > 0) {
            nd.left = leftRotate(nd.left);
            return rightRotate(nd);
        }
        if (balance > 1 && nd.bal(nd.right) >= 0)
    ...
    public static void main(String[] args) {
        T01AVLTree tree = new T01AVLTree();
        // for(int j = 1; j <= 100000; j++){
        // for(int i = 0; i < j; i++){
        // tree.head = tree.insert(tree.head, (int)(Math.random()*100000000));
        // }
        // int heig = (int)(Math.log(j+1)/Math.log(2) + 1);
        // if(j < 400 && j%10 == 0)System.out.println(j + "," + heig + "," +
        // tree.head.height);
        // if(j > 400 && j%500 == 0)System.out.println(j + "," + heig + "," +
        // tree.head.height);
        // }
