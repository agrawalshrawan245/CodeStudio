...
public class T04max_heap_sort {
    ...
    public static void main(String[] args) {
        ...
        // Heapfiy
        for (int i = 1; i < len; i++) {
            // Simple process is in max heap maximum number from the bottom moves up, till
            // it finds a suitable place.
            int j = i;
        ...
        for (int i = len - 1; i > 0; i--) {
            ...
            while (j < i) {
                if (2 * j + 1 < i && arr[j_max] < arr[2 * j + 1])
                    j_max = 2 * j + 1;
                if (2 * j + 2 < i && arr[j_max] < arr[2 * j + 2])
                    j_max = 2 * j + 2;
                if (j == j_max)
                    break;
...
// it finds a suitable place.
// while(j > 0 && arr[j] > arr[(j-1)/2]) {
// arr[j] = arr[j] + arr[(j - 1) / 2] - (arr[(j - 1) / 2] = arr[j]); //swapping
// two numbers
// j = (j - 1) / 2;
// }
// }
...
// //Swipe first and last number of heap, so that maximum number goes to the
// last.
// arr[0]=arr[0]+arr[i]-(arr[i] = arr[0]);
// int j = 0;
// int j_max = 0;
// // Arrange the top node as it is not maximum now, till it finds suitable
// place.
// // In max hip small number moves down until needed.
...
// // Result:- 0 2 3 4 6 6 7 8 9 11 13 16 44
// // This function can be further improved and heap sort can be performed only
// with help of remove function. You can check it from GFG.
// }
// }
